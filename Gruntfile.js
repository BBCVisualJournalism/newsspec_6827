...
module.exports = function (grunt) {
    ...
    grunt.registerTask('prep',         ['add_environment_data', 'prepDeploy']);
    grunt.registerTask('previewstage', ['stage_checklist', 'prep', 'replace:prepPreviewStageDeploy', 'copy:stageDeploy', 'clean:main']);
    grunt.registerTask('stage',        ['stage_checklist', 'prep', 'replace:prepStageDeploy',        'copy:stageDeploy', 'clean:main']);
    grunt.registerTask('previewlive',  ['live_checklist',  'prep', 'replace:prepPreviewLiveDeploy',  'copy:liveDeploy',  'clean:main']);
    grunt.registerTask('live',         ['live_checklist',  'prep', 'replace:prepLiveDeploy',         'copy:liveDeploy',  'clean:main']);
    ...
    grunt.registerTask('prepDeploy', [], function () {
        ...
        vocabs.forEach(function (vocab) {
            try {
                vocab_dir = fs.lstatSync(env.localhost + '/news/special/' + pkg.year + '/newsspec_' + pkg.project_number + '/' + vocab);
